...
PSSF_removals<-spdf.removals[which(st_intersects(kml_boundary, spdf.removals, sparse = FALSE)), ]
PSSF_removals<-PSSF_removals %>% arrange(Sentinel)
names<-unique(PSSF_removals$Sentinel)

#plot data
base+
  geom_sf(data = kml_boundary, aes(fill = ENTITY),inherit.aes=FALSE,show.legend = FALSE)+
  geom_sf(data = PSSF_removals, aes(fill=Sentinel), show.legend = FALSE, size=2,  inherit.aes = FALSE)
